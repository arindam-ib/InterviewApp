...
let new_interview = `
  ...
  <div id="new_interview">
    <form action="/" id="interview_form">
     ...
     <div id="drop-area">
      <div class="my-form">
        <p>Upload file with the file dialog or by dragging and dropping file onto the dashed region</p>
...
const router = () => {
    ...
    if(parsedURL == '/new_participant')
    {
      ...
      document.getElementById("participant_submit").addEventListener("click", function(event){
        ...
        const url = 'http://127.0.0.1:3001/participants';
        xhr.open("POST", url);
    ...
    if(parsedURL == '/new_interview')
    {
      ...
      document.getElementById("interview_submit").addEventListener("click", function(event){
        ...
        const url = 'http://127.0.0.1:3001/interviews';
        xhr.open("POST", url);
    ...
    {
      const xhr = new XMLHttpRequest();
      const url='http://127.0.0.1:3001/interviews/'+request.id.toString().slice(1);
      console.log(url);
      xhr.open("GET", url);
      xhr.send();
      var getData = [];
